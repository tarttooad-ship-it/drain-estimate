...
const steps = [
  ...
  },
  {
    id: "symptom",
    title: "현재 증상은 어떤가요?",
    hint: "물이 내려가는 속도와 역류 여부를 기준으로 골라주세요.",
    options: [
      { label: "천천히 내려감", icon: "↓", price: [0, 10000] },
      { label: "거의 내려가지 않음", icon: "⏳", price: [10000, 30000] },
      { label: "물이 역류함", icon: "↟", price: [30000, 70000] },
      { label: "악취가 심함", icon: "!", price: [10000, 40000] }
    ]
  },
  {
    id: "work",
  ...
  },
  {
    id: "visit",
    title: "방문 조건을 선택해주세요",
    hint: "거리에 따른 출장비와 시간대 비용을 간단히 반영합니다.",
    options: [
      { label: "일반 시간 / 가까운 지역", icon: "☀", price: [0, 0] },
      { label: "먼 지역 출장", icon: "↗", price: [20000, 50000] },
      { label: "야간 / 주말", icon: "◐", price: [30000, 80000] },
      { label: "긴급 출동", icon: "⚡", price: [50000, 120000] }
    ]
  }
];
...
function renderStep() {
  ...
  getStepOptions(step).forEach((option) => {
    ...
    button.innerHTML = `
      ...
      <span class="option-price">+ ${formatRange(option.price)}</span>
    `;
...
function showResult() {
  ...
  steps.forEach((step) => {
    ...
    row.innerHTML = `
      ...
      <strong>${formatRange(answer.price)}</strong>
    `;
